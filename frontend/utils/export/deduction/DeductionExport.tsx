...
function getEmployeeId(deduction: Deduction): string {
  if (typeof deduction.employee === "string") {
    return deduction.employee;
  }

  return deduction.employee?.empId ?? "-";
}

function getEmployeeName(deduction: Deduction): string {
  if (typeof deduction.employee === "string") {
    return "-";
  }
...
function getFatherName(deduction: Deduction): string {
  if (typeof deduction.employee === "string") {
    return "-";
  }

  return deduction.employee?.fatherName ?? "-";
}
...
function getDeductedAmount(deduction: Deduction): number {
  return Math.max(0, deduction.amount - deduction.remainingAmount);
}

function getStatusLabel(status: Deduction["status"]): string {
  switch (status) {
    case "pending":
      return "Pending";
...
function getFilterDescription(filters: DeductionFilters): string {
...
function getDeductionSummary(deductions: Deduction[]) {
  ...
  const totalDeducted = deductions.reduce(
    (total, deduction) => total + getDeductedAmount(deduction),
    0,
  );
...
export async function exportDeductionsToExcel(
  ...
  const { uniqueEmployees, totalAmount, totalDeducted, totalOutstanding } =
    getDeductionSummary(deductions);
  ...
  titleCell.value = "EMPLOYEE DEDUCTION REPORT";
  ...
  deductions.forEach((deduction, index) => {
    const deductedAmount = getDeductedAmount(deduction);

    const row = worksheet.addRow([
      getEmployeeId(deduction),
      getEmployeeName(deduction),
      getFatherName(deduction),
  ...
  const lastDataRow = 7 + deductions.length;
  ...
  const printLastRow = deductions.length > 0 ? lastDataRow + 1 : 7;
  ...
  const fileName = `deduction-report-${date}.xlsx`;
